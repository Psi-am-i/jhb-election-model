...
## Queued — eight entries: two opened 2026-09-13, six opened 2026-09-14
...
**ENTRIES 19 THROUGH 25 ARE QUEUED. Entries 1-6 are not** — they were VERIFIED IN
...
⛔ **ALL SEVEN ARE IN `pools._EMIT_DEPENDENCIES` TERRITORY AND NONE CAN BE TAKEN
OUTSIDE THE WINDOW.** Entry 19 changes `parties.py`, which is an emit dependency,
so it moves `deps_sha`; entries 20-25 change `pools.py` — a message string in
20's case, executable code in the rest — and `_code_sha` hashes both, so each
moves `pools_sha`. Entries 24 and 25 additionally move `config_sha` and
`gates_sha`. Each invalidates every spec.

⛔ **READ ENTRY 23 FIRST, AND DECIDE IT BEFORE 16 SEPTEMBER.** It is the only
row in this queue whose cost is incurred on a specific night rather than at the
next measurement, and whose damage is *invisible* — a 20-ward entrant sized as a
...
⛔ **ENTRIES 21, 22, 23 AND 25 ARE ALL FAIL-OPEN OR CRY-WOLF PATHS, AND NONE IS
A TUNING CHANGE.** Each makes a mechanism that currently reports a wrong state
confidently report the right one. **Every one was found by the same instrument —
fault injection with a working revert — and none of the four has a guard in the
suite today.** Entries 21, 23 and 25 are expected to be number-neutral and the
...
⛔ **ENTRIES 21 AND 22 ARE BOTH FAIL-OPEN PATHS, AND NEITHER IS A TUNING
CHANGE.** Both make a guard that currently scores a missing record as a safe
...
but both were found by the same instrument (fault injection with a revert) and
neither has a guard in the suite today.
