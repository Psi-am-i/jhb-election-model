...
STYLE = """
  ...
  *{box-sizing:border-box;margin:0;}
  ...
  .topnav{display:flex;flex-wrap:wrap;gap:6px 16px;align-items:baseline;
    ...
    border-bottom:1px solid var(--rule);padding-bottom:9px;margin-bottom:26px;}
...
def shell(title: str, kicker: str, masthead: str, body: str, generated_note: str) -> str:
...
def render_doc(source: Path, kicker: str, standfirst: str) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for source, (output, kicker, standfirst) in DOCS.items():
        page = render_doc(Path(source), kicker, standfirst)
