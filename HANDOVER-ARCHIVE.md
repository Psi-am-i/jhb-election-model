# Handover — superseded banners

⛔ **NOTHING HERE IS CURRENT.** These are the dated state banners that used to
sit at the top of `HANDOVER.md`, kept because they record what was believed on
their dates. `HANDOVER.md` is the file every session is sent to first, so a
superseded banner in it is a false premise delivered with the authority of the
rules. Durable findings live in `MODEL-LOG.md`, which is append-only and dated
by design; this file is the archive, and it is not maintained.

Moved out 2026-09-16.
...
## Errata moved out of the runbook
...
# The 2026-08-29 session's forward plan, superseded

⛔ **NOT CURRENT, AND PARTS ARE KNOWN FALSE.** Kept as the record of that
session. Its "POOLS-REEMIT-QUEUE holds FOUR entries" and "blocks all
multi-city 2026 work" were overtaken by events: the queue runs past twenty
and Tshwane's 2026 spec was emitted on 2026-09-08. Its review rule and
measurement discipline were promoted to `CLAUDE.md` and are read there.
