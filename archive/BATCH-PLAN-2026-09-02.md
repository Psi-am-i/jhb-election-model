...
> ## ⛔ ARCHIVED 2026-09-12 — THIS BATCH HAS LANDED. IT GOVERNS NOTHING.
>
> Every entry in this plan is in the tree. The emit it was written to schedule
> was taken on 2026-09-08 and the specs it produced are the ones on disk; see
> `../POOLS-REEMIT-QUEUE.md`, whose *Landed* table carries the grep that proves
> each entry, and `../HANDOVER.md`.
>
> It is kept as the record of what was planned and why, and it is moved here so
> that a future session cannot read a finished plan as live instruction. Its
> unticked boxes are **not** outstanding work — they went unticked because the
> plan stopped being maintained once the batch landed, which is exactly the
> failure mode `HANDOVER.md` records as *an unticked box is not evidence*.
>
> ---
>
> *The original banner, preserved:* **This is the tracked copy and the one that
> governs.** A session-local plan file is not a record: this batch spans sessions
> and a dated deliverable, so the plan and its checklist live here. Tick an item
> only when it has landed WITH its measurement beside it. **Nothing is ticked on
> intent.**
>
